...
- `Steam下载`：游戏下载与内容 CDN 强制使用 `DIRECT`，避免代理流量和跨境下载。
- `Steam平台`：Steam 商店、社区、聊天、图片等其余流量走代理策略。
- 下载规则必须位于完整 Steam 规则之前；Mihomo 按从上到下顺序匹配。
- 自建域名清单保存在 `rules/steam-download.yaml`，主配置同时嵌入同一份规则，上传单个配置文件即可使用。

下载域名参考了 Blackmatrix7 的 Steam/SteamCN 分类，但缩小到游戏内容与常见 CDN，避免把社区图片、创意工坊用户内容等全部误判为下载。
